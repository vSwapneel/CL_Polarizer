...
class SimCTGPretraining(nn.Module):
    ...
    def compute_contrastive_loss(self, cosine_scores_plus, cosine_scores_minus, margin): # 수정필요 함수 
        ...
        return cl_loss
        """
    def compute_contrastive_loss(self, cosine_sim_plus, cosine_sim_minus, margin): # 버전 3 
        '''
           margin: predefined margin to push similarity score away
           score_matrix: bsz x seqlen x seqlen; cosine similarity matrix
           input_ids: bsz x seqlen
        '''
        bsz, seqlen = cosine_sim_plus.size()
        #gold_score = torch.diagonal(score_matrix, offset=0, dim1=1, dim2=2) # bsz x seqlen
        #gold_score = torch.unsqueeze(gold_score, -1)
        #assert gold_score.size() == torch.Size([bsz, seqlen, 1])
        ...
        ### 추가 1. anchor 와 anchor_plus 를 가깝게 하고싶으면 이것 
        difference_matrix = cosine_sim_plus - cosine_sim_minus
        ### 추가 2. anchor 와 anchor_minus 를 가깝게 하고싶으면 이것 
        #difference_matrix = cosine_sim_minus - cosine_sim_plus

        assert difference_matrix.size() == torch.Size([bsz, seqlen]) ########## seqlen -> 1 
        loss_matrix = margin - difference_matrix # bsz x seqlen x seqlen
        loss_matrix = torch.nn.functional.relu(loss_matrix)
        cl_loss = torch.mean(loss_matrix)
        return cl_loss   
    ...
    def forward(self, input_ids, input_ids_plus, input_ids_minus, labels, margin): # input_ids_plus, input_ids_minus 추가 
        ...
        assert last_hidden_states.size() == torch.Size([bsz, seqlen, self.embed_dim])
        # 수정한 부분 -> logits 을 logits_plus 로.
        mle_loss = train_fct(logits_plus.view(-1, self.vocab_size), labels.view(-1))
        ...
        # 추가
        cosine_sim_plus = F.cosine_similarity(norm_rep, norm_rep_plus, dim=2)
        assert cosine_sim_plus.size() == torch.Size([bsz, seqlen])
        # 추가
        cosine_sim_minus = F.cosine_similarity(norm_rep, norm_rep_minus, dim=2)
        assert cosine_sim_minus.size() == torch.Size([bsz, seqlen])

        if self.use_cl_loss: ################# 원래는 cosine_scores_plus, cosine_scores_minus, margin
            cl_loss = self.compute_contrastive_loss(cosine_sim_plus, cosine_sim_minus, margin) 
        else:
            cl_loss = torch.Tensor([0.])
            if input_ids.is_cuda:
                cl_loss = cl_loss.cuda(input_ids.get_device())
